    ...
    <div class="bg-white shadow rounded-lg overflow-auto max-w-2xl mx-auto">
        ...
        <form action="{{ route('admin.user-licenses.update', $userLicense) }}" method="POST" class="p-6 space-y-6">
            ...
            <div x-data="{
                isOpen: false,
                searchQuery: '',
                selectedEmployee: {{ $userLicense->employee_id }},
                ...
                <div class="relative">
                    <button type="button"
                        ...
                        <span class="block truncate" x-text="getSelectedEmployeeName()"></span>
            ...
            <div x-data="{
                isOpen: false,
                searchQuery: '',
                selectedLicense: {{ $userLicense->license_id }},
                licenses: {{ json_encode($licenses->map(fn($l) => [
                    'id' => $l->id,
                    'name' => $l->license_name,
                ...
                get filteredLicenses() {
                    ...
                    return this.licenses.filter(lic =>
                        lic.name.toLowerCase().includes(this.searchQuery.toLowerCase()) ||
                        lic.vendor.toLowerCase().includes(this.searchQuery.toLowerCase())
                    );
                },
                selectLicense(licId) {
                ...
                getSelectedLicenseName() {
                    const lic = this.licenses.find(l => l.id === this.selectedLicense);
                    return lic ? lic.name + ' (' + lic.vendor + ')' : 'Select License';
                }
            }">
                <label class="block text-sm font-medium text-gray-700 mb-1">
                ...
                <input type="hidden" name="license_id" :value="selectedLicense">

                <div class="relative">
                    <button type="button"
                        @click="isOpen = !isOpen"
                        class="relative w-full bg-white border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-pointer focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        <span class="block truncate" x-text="getSelectedLicenseName()"></span>
                        <span class="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none">
                            <svg class="h-5 w-5 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                <path fill-rule="evenodd" d="M10 3a1 1 0 01.707.293l3 3a1 1 0 01-1.414 1.414L10 5.414 7.707 7.707a1 1 0 01-1.414-1.414l3-3A1 1 0 0110 3zm-3.707 9.293a1 1 0 011.414 0L10 14.586l2.293-2.293a1 1 0 011.414 1.414l-3 3a1 1 0 01-1.414 0l-3-3a1 1 0 010-1.414z" clip-rule="evenodd" />
                            </svg>
                    ...
                    <div x-show="isOpen"
                        ...
                        <template x-for="lic in filteredLicenses" :key="lic.id">
                            <div @click="selectLicense(lic.id)"
                                 :class="{'bg-indigo-50': selectedLicense === lic.id}"
                                 class="cursor-pointer select-none relative py-2 pl-3 pr-9 hover:bg-indigo-50">
                                <div class="flex flex-col">
                                    <span class="font-medium block truncate" x-text="lic.name"></span>
                                    <span class="text-gray-500 text-xs" x-text="lic.vendor"></span>
                                </div>
                                <span x-show="selectedLicense === lic.id" class="absolute inset-y-0 right-0 flex items-center pr-4 text-indigo-600">
                                    <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                        <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
                                    </svg>
                                </span>
                        ...
                        <div x-show="filteredLicenses.length === 0" class="py-2 px-3 text-sm text-gray-500">
                            No licenses found
                        </div>
                    </div>
                </div>
