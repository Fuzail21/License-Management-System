...
@section('content')
    <div class="bg-white shadow rounded-lg overflow-hidden max-w-2xl mx-auto">
        <div class="px-5 py-4 border-b border-gray-200">
            <h3 class="text-lg leading-6 font-medium text-gray-900">Edit Division</h3>
        </div>
            ...
            <div>
                ...
                <div class="mt-1">
                    <textarea name="description" id="description" rows="4"
                        class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md p-2 border"
                        placeholder="Optional description of the division's responsibilities and scope">{{ old('description', $division->description) }}</textarea>
                </div>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                ...
                <div class="mt-1">
                    <select name="gm_id" id="gm_id"
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('gm_id', $division->gm_id) == $user->id ? 'selected' : '' }}>
                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        <p class="text-sm text-yellow-700">
                            <strong>Warning:</strong> Changing division details will affect all departments assigned to this division.
                        </p>
                    </div>
                </div>
            </div>
