...
@section('content')
    <div class="bg-white shadow rounded-lg overflow-hidden max-w-2xl mx-auto">
        <div class="px-5 py-4 border-b border-gray-200">
            <h3 class="text-lg leading-6 font-medium text-gray-900">Edit Renewal</h3>
        </div>
        <form action="{{ route('admin.renewals.update', $renewal) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
                ...
                <div class="mt-1">
                    <select id="license_id" name="license_id" required
                        ...
                        @foreach($licenses as $license)
                            <option value="{{ $license->id }}" {{ old('license_id', $renewal->license_id) == $license->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2">
                <div>
                    ...
                    <div class="mt-1">
                        <input type="date" name="renewal_date" id="renewal_date"
                            value="{{ old('renewal_date', $renewal->renewal_date->format('Y-m-d')) }}" required
                            class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md p-2 border">
                    </div>
                    @error('renewal_date')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <input type="date" name="new_expiry_date" id="new_expiry_date"
                            value="{{ old('new_expiry_date', $renewal->new_expiry_date->format('Y-m-d')) }}" required
                            class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md p-2 border">
                    </div>
                ...
                <div>
                    ...
                    <div class="mt-1 relative rounded-md shadow-sm">
                        ...
                        <input type="number" name="cost" id="cost" value="{{ old('cost', $renewal->cost) }}" step="0.01"
                            required
            ...
            <div>
                ...
                <div class="mt-1">
                    <textarea id="notes" name="notes" rows="3"
                        class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md p-2 border">{{ old('notes', $renewal->notes) }}</textarea>
                </div>
                @error('notes')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
